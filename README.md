...
If Redis runs on a dynamically mapped port (e.g. `docker port` or `docker compose port`), first determine the host port and then point `REDIS_URL` to it:
   ...
   Logs for scrape jobs and generated knowledge base markdown are mounted from the host at
   `./logs/scrape_jobs` and `./public/knowledge_base` so rebuilds do not discard historic runs. When
   you delete a scrape job, its log and knowledge-base outputs under these directories are removed
   by the app, regardless of whether the job was created on the host or in Docker.
